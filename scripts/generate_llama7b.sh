#!/bin/bash
#SBATCH --job-name=gen_llama7b # create a short name for your job
#SBATCH -p batch
#SBATCH -x dgx047
...
CODE_ROOT="./tools/generate_by_hf.py"
hf_model_path="/cognitive_comp/common_data/Huggingface-Models/llama_7b"
prompt="write python code of bubble sort."

# prompt="这是一个装满爆米花的袋子，袋子里没有巧克力。 然而，袋子上的标签上写着“巧克力”而不是“爆米花”。 山姆找到了包。 她以前从未见过这个包。 她看不到袋子里装的是什么。 她读了标签。然后她打开包查看里面的物品，她看到包里都是？"
# prompt="怎么实现财富自由？"
# prompt="北京是中国的"
srun python $CODE_ROOT --hf_model_path $hf_model_path --prompt "$prompt" --model_type "llama" --max_length 300
