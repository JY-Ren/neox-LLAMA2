...
def write_model(model_path, input_base_path, model_size,num_shards,tokenizer_path):
    ...
    word_embeddings = torch.cat([t['word_embeddings.weight'] for t in load_partitions(input_base_path,num_shards,0)],dim=0)
    ...
    lm_head = torch.cat([t['final_linear.weight'] for t in load_partitions(input_base_path,num_shards,n_layers+4)],dim=0)
    # Unsharded
    state_dict = {
