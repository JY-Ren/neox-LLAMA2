...
def convert_model_pipeline(
    ...
    num_output_shards_pipeline: int,
):
    ...
    for f in file_path_list:
        if ".bin" in f and "pytorch_model" in f and "json" not in f:
            print(f)
            ...
            for k, v in tmp.items():
                print(k, v.shape)
                ckpt[k] = v
            # pbar.set_description(f"Loaded shard {f}")
            # pbar.update(1)
    ...
    embeddings_in = torch.cat(
        [
            loaded[rank]["model.embed_tokens.weight"].cpu()
            for rank in range(num_input_shards)
        ],
        dim=1,
    )
    helper.save_shards(
        {"word_embeddings.weight": helper.shard(embeddings_in[:vocab_size, :], dim=0)},
        layer_i=0,
    )
    helper.del_loaded("model.embed_tokens.weight")
    pbar.set_description(f"Saved embeddings")
    pbar.update(1)

    # Norms
    helper.save_duplicates(
        {"norm.scale": loaded[0]["model.norm.weight"]}, layer_i=num_layers + 3
    )
    helper.del_loaded("model.norm.weight")
    pbar.set_description(f"Saved final norm")
    pbar.update(1)

    # Embedding out
    embeddings_out = torch.cat(
        [loaded[rank]["lm_head.weight"].cpu() for rank in range(num_input_shards)],
        dim=0,
    )
    helper.save_shards(
        {"final_linear.weight": helper.shard(embeddings_out[:vocab_size, :], dim=0)},
        layer_i=num_layers + 4,
    )
    helper.del_loaded("lm_head.weight")
    pbar.set_description(f"Saved out embeddings")
    pbar.update(1)

    # Layers
    # if model_size == "7B":
    rope_freqs = loaded[0]["model.layers.0.self_attn.rotary_emb.inv_freq"]
    helper.del_loaded("model.layers.0.self_attn.rotary_emb.inv_freq")
    # else:
    ...
    for layer_i in range(num_layers):
        ...
        attn_wo = helper.shard(
            torch.cat(
                [
                    loaded[rank][f"model.layers.{layer_i}.self_attn.o_proj.weight"]
                    for rank in range(num_input_shards)
                ],
                dim=1,
            ),
        ...
        mlp_w1 = helper.shard(
            torch.cat(
                [
                    loaded[rank][f"model.layers.{layer_i}.mlp.gate_proj.weight"]
                    for rank in range(num_input_shards)
                ],
                dim=0,
            ),
        ...
        mlp_w2 = helper.shard(
            torch.cat(
                [
                    loaded[rank][f"model.layers.{layer_i}.mlp.down_proj.weight"]
                    for rank in range(num_input_shards)
                ],
                dim=1,
            ),
        ...
        mlp_w3 = helper.shard(
            torch.cat(
                [
                    loaded[rank][f"model.layers.{layer_i}.mlp.up_proj.weight"]
                    for rank in range(num_input_shards)
                ],
                dim=0,
            ),
        ...
        helper.del_loaded(f"model.layers.{layer_i}.self_attn.o_proj.weight")
        helper.del_loaded(f"model.layers.{layer_i}.mlp.gate_proj.weight")
        helper.del_loaded(f"model.layers.{layer_i}.mlp.down_proj.weight")
        helper.del_loaded(f"model.layers.{layer_i}.mlp.up_proj.weight")
        ...
        w_q = permute_rotary(
            torch.cat(
                [
                    ...
                    ].view(num_heads_per_input_shard, dims_per_head, hidden_size)
                    for rank in range(num_input_shards)
                ],
                dim=0,
            )
        ...
        w_v = torch.cat(
            [
                loaded[rank][f"model.layers.{layer_i}.self_attn.v_proj.weight"].view(
        ...
        helper.del_loaded(f"model.layers.{layer_i}.self_attn.q_proj.weight")
        helper.del_loaded(f"model.layers.{layer_i}.self_attn.k_proj.weight")
        helper.del_loaded(f"model.layers.{layer_i}.self_attn.v_proj.weight")

        # Duplicated
        input_layernorm = loaded[0][f"model.layers.{layer_i}.input_layernorm.weight"]
        post_attention_layernorm = loaded[0][
            f"model.layers.{layer_i}.post_attention_layernorm.weight"
        ]
        helper.del_loaded(f"model.layers.{layer_i}.input_layernorm.weight")
        helper.del_loaded(f"model.layers.{layer_i}.post_attention_layernorm.weight")
    ...
    model_state = {
        "dp_world_size": 1,
        "mp_world_size": num_output_shards * num_output_shards_pipeline,
        "module": None,
        "optimizer": None,
        "global_steps": 1,
        "skipped_steps": 1,
    ...
    for rank in range(num_output_shards * num_output_shards_pipeline):
        torch.save(
...
def convert_model_sequential(
    ...
    for rank in range(num_output_shards):
        module = {}
        for key in helper.sequential_cache[rank]:
            module[key.replace("sequential.", "")] = helper.sequential_cache[rank][key]
        model_state = {
            "dp_world_size": 1,
...
def main():
    ...
    if args.pipeline_parallel:
        ...
        convert_model_pipeline(
            ...
            num_output_shards_pipeline=args.num_output_shards_pipeline,
        )
    else:
        print("sequential")
