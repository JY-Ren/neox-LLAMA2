...
def add_extra_tokens(embeddings, extra_tokens):    
    embeddings_avg = embeddings.mean(dim=0, keepdim=True).repeat(extra_tokens, 1)
    print(f"add extra tokens to embeddings, raw shape: {embeddings.shape}")
    embeddings = torch.cat([embeddings, embeddings_avg], dim=0)
...
def convert_model_pipeline(
    ...
    print("num_heads ", num_heads)  # 64
    num_heads_per_input_shard = num_heads // num_input_shards
    num_heads_per_output_shard = num_heads // num_output_shards  # tesor shards
    hidden_size = params["dim"]
    dims_per_head = hidden_size // num_heads  # 8192/64=128，每个head 128维
    base = 10000.0
    inv_freq = 1.0 / (
    ...
    if "n_kv_heads" in params:
        ...
        print("num_key_value_heads ", num_key_value_heads)  # 8
        
        num_local_kv_heads_per_input_shard = num_key_value_heads // num_input_shards
        print("num_local_kv_heads_per_input_shard ", num_local_kv_heads_per_input_shard)  
        num_local_kv_heads_per_output_shard = num_key_value_heads // num_output_shards
        print("num_local_kv_heads_per_output_shard ", num_local_kv_heads_per_output_shard)  
        

    def permute_rotary(w):
        # assert w.shape == (num_heads, dims_per_head, hidden_size)
        local_num_heads, dims_per_head, hidden_size = w.shape
        return (
            w.view(local_num_heads, dims_per_head // 2, 2, hidden_size)
            .transpose(1, 2)
            .reshape(local_num_heads, dims_per_head, hidden_size)
        )
    ...
    if extra_tokens > 0:
        embeddings_in = add_extra_tokens(embeddings_in, extra_tokens)
    ...
    if extra_tokens > 0:
        embeddings_out = add_extra_tokens(embeddings_out,extra_tokens)
    ...
    for layer_i in range(num_layers):
            ...
            torch.cat(
                [
                    loaded[rank][f"layers.{layer_i}.attention.wk.weight"].view(
                        num_local_kv_heads_per_input_shard, dims_per_head, hidden_size
                    )  # 1, 128, 8192
                    for rank in range(num_input_shards)
                ],
                dim=0,
        ...
        print(helper.shard(w_q, dim=0).shape, 
              helper.shard(w_k, dim=0).shape, 
              helper.shard(w_v, dim=0).shape,)
        ...
        q = helper.shard(w_q, dim=0) # num_output_shards, num_heads_per_output_shard, dims_per_head, hidden_size
        q = q.view(
            num_output_shards,
        ...
        sharded_kv = sharded_kv.view(
            num_output_shards,
            num_local_kv_heads_per_output_shard * 2 * dims_per_head,
            hidden_size,
        )  # 4, 1*2*2*128, 8192
        print("sharded_kv.shape: ", sharded_kv.shape) # [4, 512, 8192]
        ...
        for out_rank in range(num_output_shards):
            helper.save(
                {
                    "attention.query.weight": q[out_rank],
                    "attention.key_value.weight": sharded_kv[out_rank],
                    ...
                    "attention.rotary_emb.inv_freq": inv_freq,
                },
                layer_i=layer_i + 2,
                rank=out_rank,
            )
    ...
    def add_sequential_shard(self, dictionary, layer_i):
        ...
        for k, v in dictionary.items():
            for rank in range(self.num_output_shards):
                ...
                self.sequential_cache[rank][f"{layer_i}.{k}"] = v[rank].clone()
    ...
    def add_sequential_duplicates(self, dictionary, layer_i):
        ...
        for k, v in dictionary.items():
            for rank in range(self.num_output_shards):
                ...
                self.sequential_cache[rank][f"{layer_i}.{k}"] = v.clone()
...
def main():
    ...
    parser.add_argument(
        "--model_size",
        choices=["7B", "13B", "70B", "tokenizer_only"],
    )
    parser.add_argument(
        "--output_dir",
    ...
    if args.pipeline_parallel:
        ...
        raise "invalid args.pipeline_parallel {args.pipeline_parallel}"
