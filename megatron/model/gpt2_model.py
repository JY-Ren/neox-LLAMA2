...
def _pre_transformer_block_residual(args):
    # data format change for hidden_states to avoid explicit tranposes : [b s h] --> [s b h]
    assert len(args) == 2, "Incorrect number of arguments to _pre_transformer_block"
    fn = lambda _args: (
        (
            _args[0][0].transpose(0, 1).contiguous(),
            _args[0][1].transpose(0, 1).contiguous(),
        ),
        *_args[1:],
    )
    return fn(args)
...
def _post_transformer_block_residual(args):
    # from (hidden_states, attention_mask)
    # to (hidden_states.T)
    assert len(args) == 2, "Incorrect number of arguments to _post_transformer_block"
    fn = lambda _args: (
...
class GPT2ModelPipe(PipelineModule, torch.nn.Module):
    ...
    def init_specs(self):
        ...
        if self.residual_norm:
            self.specs.append(_pre_transformer_block_residual)
        else:
        ...
        if self.residual_norm:
            self.specs.append(_post_transformer_block_residual)
        else:
