...
class DynamicRotaryEmbedding(torch.nn.Module):
    '''
    reference: 
    https://www.reddit.com/r/LocalLLaMA/comments/14mrgpr/dynamically_scaled_rope_further_increases/
    https://github.com/jquesnelle/scaled-rope/blob/master/scaled_rope/LlamaDynamicScaledRotaryEmbedding.py
    '''
    def __init__(self, dim, base=10000,  precision=torch.half, ntk=False, ori_position_embeddings=2048):
        super().__init__()
        self.dim = dim
        self.ntk = ntk
        self.base = base
        self.ori_position_embeddings = ori_position_embeddings
        inv_freq = 1.0 / (base ** (torch.arange(0, dim, 2).float() / dim))
        self.register_buffer("inv_freq", inv_freq)
    ...
    def forward(self, x, seq_dim=1, seq_len=None):
        ...
        if seq_len != self.seq_len_cached:
            ...
            if self.ntk:
                base = self.base * ((self.ntk * seq_len / self.ori_position_embeddings) - (self.ntk - 1)) ** (self.dim / (self.dim-2))
                inv_freq = 1.0 / (base ** (torch.arange(0, self.dim, 2).float().to(x.device) / self.dim))
                self.register_buffer("inv_freq", inv_freq)
            t = torch.arange(seq_len, device=x.device).float()
            if not self.ntk:
                t *= self.ori_position_embeddings / seq_len
