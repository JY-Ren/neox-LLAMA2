...
def make_data_loader(datasets, neox_args):
    """Build dataloader given an or a list of input dataset."""
    def _make_data_loader(dataset, neox_args):
        if dataset is None:
            return None
    ...
    # Torch dataloader.
    if neox_args.valid_by_datasets and datasets and isinstance(datasets, list): # rank 0,3 last stage in pp
        return [_make_data_loader(ds, neox_args) for ds in datasets]
    else:
        return _make_data_loader(datasets, neox_args)  
...
def extend_indexmap_path(target_data_paths,indexmap_paths):
    '''
    理论上len(neox_args.train_indexmap_data_paths)==len(neox_args.train_data_paths)时zip_longest才不会生成值为None的train_indexmap_data_prefix，
    test 和val数据类似，此种情况会导致indexmap_data存放位置混乱，因此当index_map路径中只有一个文件时，自动重复对齐到对应的数据的文件数 
    '''
    assert  len(indexmap_paths) == 1 or len(indexmap_paths) == len(target_data_paths) #要么只指定一个，要么就挨个指定
    indexmap_paths = indexmap_paths + (len(target_data_paths)-len(indexmap_paths))*[indexmap_paths[0]]
    return indexmap_paths
...
def build_weighted_datasets(
    ...
    for i, (
        ...
        valid_path,
        test_path,
        train_indexmap_data_prefix,
        valid_indexmap_data_prefix,
        test_indexmap_data_prefix,
    ) in enumerate(
        zip_longest(
            neox_args.train_data_paths,
            neox_args.valid_data_paths,
            neox_args.test_data_paths,
            extend_indexmap_path(neox_args.train_data_paths,neox_args.train_indexmap_data_paths),
            extend_indexmap_path(neox_args.valid_data_paths,neox_args.valid_indexmap_data_paths),
            extend_indexmap_path(neox_args.test_data_paths,neox_args.test_indexmap_data_paths),
        )
    ):
        if train_path:
...
def build_train_valid_test_data_iterators(neox_args):
    ...
    if mpu.get_model_parallel_rank() == 0 and pipe_load:
        ...
        if neox_args.train_data_paths:
            ...
            if valid_datasets:
                # BlendableDataset will merge all valid datasets with weights
                if neox_args.valid_by_datasets:  
                    valid_ds = valid_datasets
                else:
            ...
            # when valid_ds is splited from all, cannot valid by datasets
            if neox_args.valid_by_datasets:
                print_rank_0(
                    "WARNING: valid datasets is splited from data_path and cannot valid by datasets"
                )
    ...
    if valid_dataloader is not None:
        ...
        if neox_args.valid_by_datasets and valid_dataloader:
            for valid_dl in valid_dataloader:
                valid_dl.batch_sampler.start_iter = start_iter_val % len(valid_dl)
                print_rank_0(
                    "setting validation data start iteration to {}".format(
                        start_iter_val % len(valid_dl)
                    )
                )
        else:
    ...
    if valid_dataloader is not None:
        if neox_args.valid_by_datasets:
            valid_data_iterator = [iter(valid_dl) for valid_dl in valid_dataloader if valid_dl is not None]    
        else:
            valid_data_iterator = iter(valid_dataloader)
    else:
        if neox_args.valid_by_datasets:
            valid_data_iterator = [None for i in range(len(neox_args.valid_data_paths))] # to avoid NCCL error when validate
        else:
