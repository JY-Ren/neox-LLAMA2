...
def training_log(
    ...
    if neox_args.log_optimizer_states:
        # print_rank_0('optimizer.state_dict():\n',optimizer.state_dict())
        # if "optimizer_state_dict" in optimizer.state_dict():
        for k, v in optimizer.state_dict()["base_optimizer_state"]["state"][
            "0"
        ].items():
    ...
    if (
        ...
        or neox_args.log_grad_info
    ):
        if neox_args.log_grad_pct_zeros or neox_args.log_grad_norm or neox_args.log_grad_info:
        ...
        for i, (name, param) in enumerate(model.module.named_parameters()):
            ...
            #     )
            log_map = {}
            if neox_args.log_grad_info or neox_args.log_grad_norm or neox_args.log_grad_pct_zeros:
                if (
                    hasattr(model,"stored_gradients") and model.stored_gradients is not None
                ):
                    grad = model.stored_gradients[i]
                    if grad is not None:
                        if neox_args.log_grad_info:
                            log_map.update({
                                f"grad/max-{name}": grad.float().max().item(),
                                f"grad/min-{name}": grad.float().min().item(),
                                f"grad/mean-{name}":grad.float().mean().item(),
                                f"grad/std-{name}":grad.float().std().item()
                            })
                        if neox_args.log_grad_pct_zeros:
                            log_map[f"grad/pct_zeros/{name}"] = (grad == 0).float().mean().item() * 100
                        if neox_args.log_grad_norm:
                            log_map[f"grad/norms/{name}"] = torch.norm(grad)
                        
            if neox_args.log_param_info:
                log_map.update({
                    f"param/max-{name}": param.float().max().item(),
                    f"param/min-{name}": param.float().min().item(),
                    f"param/mean-{name}":param.float().mean().item(),             
                    f"param/std-{name}":param.float().mean().item()         
                }) 
            if neox_args.log_param_norm:
                log_map[f"param/norm-{name}"] = torch.norm(param)
