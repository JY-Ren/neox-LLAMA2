...
class NeoXArgs(*BASE_CLASSES):
    ...
    def consume_neox_args(cls, overwrite_values=None, args=None):
        ...
        # the last item of get_deepspeed_main_args is megatron_config path
        args_parsed.megatron_config = args[-1]
        # print("args_parsed.megatron_config:", args_parsed.megatron_config)

        # avoid some node not load megatron_config.json
        while not os.path.exists(args_parsed.megatron_config):
            sleep(2)
        
        sleep(2) #确定config文件存在后，继续sleep两秒，确保config文件被写完成，否则有可能报json文件解析错误
    ...
    def get_deepspeed_main_args(self):
        ...
        # we can set different magatron_config.json for different path.
        megatron_fp = self.megatron_config_path
        ...
        if self.rank == 0:
            # if megatron_fp exist, delete it.
            if os.path.exists(megatron_fp):
                os.remove(megatron_fp)
    ...
    def validate_values(self):
        ...
        if self.valid_data_paths is not None:
            ...
            if self.valid_data_names is not None:
                assert len(self.valid_data_paths) == len(self.valid_data_names)
            else:
                self.valid_data_names = [str(idx) for idx in range(0,len(self.valid_data_weights))]
