...
from megatron.utils import (
    ...
    get_ltor_masks_and_position_ids_with_reset,
    get_loss_masks_with_reset,
    reduce_losses,
)
...
from megatron.data.finetune_dataset import (
    build_finetune_train_valid_test_data_iterators,
)
...
def pretrain(neox_args):
    ...
    if neox_args.use_finetune_dataset:
        (
            train_data_iterator,
            valid_data_iterator,
            test_data_iterator,
        ) = build_finetune_train_valid_test_data_iterators(neox_args=neox_args)
    else:
    ...
    if not neox_args.valid_by_datasets:
    ...
    if neox_args.do_valid:  
        if valid_data_iterator:
            prefix = "iteration {}".format(iteration)
            for idx, each_iterator in enumerate(valid_data_iterator):
                chart_name = neox_args.valid_data_names[idx] if neox_args.valid_data_names else str(idx)
                evaluate_and_print_results(
                    neox_args=neox_args,
                    prefix=prefix,
                    forward_step_func=forward_step,
                    data_iterator=each_iterator,
                    model=model,
                    iteration=iteration,
                    verbose=False,
                    timers=timers,
                    chart_name="validation_"+chart_name
                ) 
...
def _get_batch(neox_args, tokenizer, keys, data, datatype):
    ...
    if neox_args.use_finetune_dataset:
        keys.extend(["loss_mask", "position_ids", "attention_mask"])
    ...
    if neox_args.use_finetune_dataset:
        ...
        attention_mask = data_b["attention_mask"].unsqueeze(dim=1)
        attention_mask = attention_mask < 0.5
        position_ids = data_b["position_ids"].long()

    if neox_args.reset_loss_mask:
        loss_mask = get_loss_masks_with_reset(
            data=tokens,
            eod_token=neox_args.tokenizer.eod,
            bot_token=neox_args.bot_token_id,
            human_token=neox_args.human_token_id,
        )

    if neox_args.reset_human2eod:
        labels = labels.clone()
        labels[labels == neox_args.human_token_id] = neox_args.tokenizer.eod

    # print the sample at start and random it
    global SHOW_DATA
    if SHOW_DATA and random() > 0.6:
        SHOW_DATA = False
        ...
        #     print_rank_0(
        #         "position: {}, loss_mask: {}, token: {}, label: {}".format(
        #             position_ids[0, t], loss_mask[0, t], tokens[0, t], labels[0, t]
        #         )
        #     )
...
def setup_model_and_optimizer(neox_args, use_cache=False, iteration=None):
    ...
    if neox_args.deepspeed:
        ...
        ds_config = neox_args.deepspeed_config
        ds_config["gradient_clipping"] = neox_args.gradient_clipping
