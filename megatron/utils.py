...
# get reset attention mask and position id, used for max_len combined with multiple sample.
def get_ltor_masks_and_position_ids_with_reset(
    data,
    eod_token,
    eod_mask_loss=False,
...
def get_loss_masks_with_reset(
    ...
    for b in range(micro_batch_size):
        ...
        for j in range(eod_index.size()[0]):
            i = eod_index[j]
            human_index = position_ids[b, cur : i + 1][
                data[b, cur : i + 1] == human_token
            ]
            bot_index = (
                position_ids[b, cur : i + 1][data[b, cur : i + 1] == bot_token] + 1
            )
            if bot_index.shape[0] == human_index.shape[0] and bot_index.shape[0] != 0:
                for bidx in range(bot_index.size()[0]):
                    if human_index.shape[0] > bidx + 1:
                        loss_mask[b, bot_index[bidx] : human_index[bidx + 1]] = 1
                    else:
                        loss_mask[b, bot_index[bidx] : i] = 1
            cur = i + 1
    return loss_mask
